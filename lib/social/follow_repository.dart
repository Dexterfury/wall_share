...
class FollowRepository {
  ...
  Future<void> followUser(String currentUserId, String userIdToFollow) async {
    try {
      ...
      final targetUserRef =
          _firestore.collection(Constants.usersCollection).doc(userIdToFollow);
      ...
      batch.set(
          followersRef, {Constants.createdAt: FieldValue.serverTimestamp()});

      // set the followings count for the current user
      batch.set(
          followingsRef, {Constants.createdAt: FieldValue.serverTimestamp()});

      // Update the followers count for the target user
      batch.update(
          targetUserRef, {Constants.followers: FieldValue.increment(1)});

      // Update the followings count for the current user
      batch.update(
          currentUserRef, {Constants.following: FieldValue.increment(1)});
