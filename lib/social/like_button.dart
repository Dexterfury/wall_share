...
class _LikeButtonState extends State<LikeButton>
  ...
  void initState() {
    ...
    _scaleAnimation = TweenSequence<double>([
      TweenSequenceItem(
        tween: Tween<double>(begin: 1, end: 1.8),
        weight: 50,
      ),
      TweenSequenceItem(
        tween: Tween<double>(begin: 1.8, end: 1),
        weight: 20,
      ),
  ...
  void _handleLike() async {
    if (widget.uid.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Please login to like wallpaper',
          ),
        ),
      );
      return;
    ...
    try {
      await _likeRepository.toggleLike(
        widget.wallpaperId,
        widget.uid,
      );
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'There was an error liking the wallpaper',
            ),
          ),
        );
      }
